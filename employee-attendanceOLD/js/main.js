...
document.addEventListener('DOMContentLoaded', function() {
    ...
    proceedBtn.addEventListener('click', (e) => {
        ...
        // Navigate directly since user is already authenticated (no face capture needed)
    ...
    // Handle password login (complete authentication without camera)
    async function handlePasswordLogin() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Complete login immediately without camera
                await completeLoginWithoutCamera(result.user);
    ...
    // Complete login without camera - show user image and navigation buttons
    async function completeLoginWithoutCamera(user) {
        try {
            // Set up session
            ...
            // Show face section with user image instead of camera
            showUserImageAndNavigation(user);
            ...
            showError('Login completion failed. Please try again.');
        }
    }

    // Show user image and navigation buttons
    function showUserImageAndNavigation(user) {
        // Show face section
        faceSection.style.display = 'block';
        
        // Hide camera elements and show user image
        ...
        // Show appropriate navigation button immediately
    ...
    async function handleFaceCapture() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Update navigation based on user role after face capture
                if (authenticatedUser) {
                    console.log('Updating navigation after face capture for user:', authenticatedUser.id);
                    updateNavigationForRole(authenticatedUser);
                } else {
                    // Fallback: Show proceed button if no authenticated user yet
                    proceedBtn.style.display = 'block';
                    proceedBtn.disabled = false;
                    console.log('No authenticated user yet, showing default proceed button');
                }
